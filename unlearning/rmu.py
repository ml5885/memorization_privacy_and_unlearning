...
def train_rmu_unlearning(tokenizer, model, forget_records, refusal, U, alpha, epochs, lr, batch_size, layer, save_dir):
    ...
    def collate(batch):
        ...
        ids_pad = torch.stack([torch.nn.functional.pad(x, (0, maxlen - x.size(0)), value=pad_id) for x in ids], dim=0)
        attn_pad = torch.stack([torch.nn.functional.pad(x, (0, maxlen - x.size(0)), value=0) for x in attn], dim=0)
        labels_pad = ids_pad.clone()
        
        return {"input_ids": ids_pad, "attention_mask": attn_pad, "labels": labels_pad}

    dl = DataLoader(data, batch_size=batch_size, shuffle=True, collate_fn=collate, drop_last=False)
